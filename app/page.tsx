...
export default function Home() {
    ...
    fetch(`${API_BASE}/api/courses`)
      ...
      .then((data) => {
        ...
        if (Array.isArray(data)) {
            ...
            console.error("API returned bad data for courses:", data);
            setCourses([]); // Keep empty to prevent crash
        }
      })
      .catch((err) => {
  ...
  const fetchTests = () => {
    fetch(`${API_BASE}/api/tests`)
      .then((res) => res.json())
      .then((data) => {
        // SAFETY CHECK: Only set tests if data is actually an Array
        if (Array.isArray(data)) {
            setTests(data);
        } else {
            console.error("API returned bad data for tests:", data);
            setTests([]); 
        }
      })
      .catch((err) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {courses.length === 0 ? (
                <div className="col-span-full text-center py-20 text-gray-400">
                  ...
                  <p className="text-sm">Please ask Admin to upload content.</p>
                </div>
              ) : (
                courses.map((course) => (
                  <div key={course.id || Math.random()} className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-lg transition flex flex-col overflow-hidden">
                    <div className="h-44 bg-gray-300 relative">
                      <img src={course.thumbnail_url || "https://placehold.co/600x400"} alt="Course" className="w-full h-full object-cover"/>
                    </div>
                    <div className="p-4 flex flex-col flex-1">
                      <h4 className="font-bold text-gray-900 line-clamp-2 mb-2">{course.title || "Untitled Course"}</h4>
